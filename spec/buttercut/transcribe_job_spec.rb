...
RSpec.describe TranscribeJob do
  ...
  # otherwise.
  before { allow(MediaTools).to receive(:audio_stream?).and_return(true) }
  ...
  # The fake whisperx writes `<basename>.json` into whatever --output_dir it was
  # given — the job runs it in a per-job tempdir and moves the result to the
  # canonical transcript_path.
  ...
  it 'moves the tempdir transcript to the canonical transcript_path' do
    stub_whisperx
    allow(job).to receive(:prepare_transcript)
  ...
  it 'honours a custom transcript_name (audio clips flatten their extension)' do
    named = described_class.new(
      ...
      video_path: '/footage/clip.wav', output_dir: output_dir,
      language_code: 'en', whisper_model: 'small',
      transcript_name: 'clip_wav.json'
    )
    # WhisperX names its output after the input file (clip.json), not our convention.
    stub_whisperx
    allow(named).to receive(:prepare_transcript)

    named.perform

    expect(File.exist?(File.join(output_dir, 'clip_wav.json'))).to be(true)
  end
