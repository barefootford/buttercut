...
RSpec.describe ButterCut::FCP7 do
  ...
  describe 'mixed frame rate clips' do
    ...
    let(:mixed_metadata) do
      {
        clip_30fps_path => build_metadata(duration_seconds: 10.0, frame_rate: '30/1'),
        clip_50fps_path => build_metadata(duration_seconds: 8.0,  frame_rate: '50/1')
      }
    end
    ...
    it 'uses sequence timebase for clipitem in/out, not file native rate' do
      ...
      # If using file rate 30fps: in would be 60, out 150 — wrong.
      ...
      # If using file rate 50fps: in would be 50, out 250 — wrong.
    ...
    it 'uses file native rate for file-level duration' do
      ...
      # 30fps clip: 10s * 30 = 300 frames; 50fps clip: 8s * 50 = 400 frames
  ...
  describe 'motion filter for portrait sequences' do
    ...
    it 'uses clip-level rotation override when file has no rotation metadata' do
      ...
      # rotation: 90 in override → fcp_rotation = -90 (CW in FCP7) once PR #34's rotation
      # filter is merged. Until then, the override at least flows through asset_map.
      ...
      xml = generator.to_xml
      # Override flowed through: motion filter present (because effective dims swap)
      expect(xml).to include('<name>Basic Motion</name>')
    end
  end
  ...
  describe 'speed (time remap) option' do
    it 'adds Time Remap filter when clip has speed != 1.0' do
      generator = described_class.new([{ path: clip_a_path, speed: 0.95 }])
      xml = generator.to_xml
    ...
    it 'does not add Time Remap filter when speed is 1.0' do
      generator = described_class.new([{ path: clip_a_path, speed: 1.0 }])
      xml = generator.to_xml
      expect(xml).not_to include('timeremap')
    end

    it 'calculates correct source frames for slow-down (speed < 1.0)' do
      # clip_a is 4 seconds at 25fps = 100 timeline frames; at 0.5 speed, 50 source frames consumed.
      generator = described_class.new([{ path: clip_a_path, speed: 0.5 }])
      xml = generator.to_xml
    ...
    it 'calculates correct source frames for speed-up (speed > 1.0)' do
      # clip_b: start_at 1.0s, duration 2.0s = 50 timeline frames; at 1.5x = 75 source frames.
      generator = described_class.new([{ path: clip_b_path, start_at: 1.0, duration: 2.0, speed: 1.5 }])
      xml = generator.to_xml
      # source in = 25 (1.0s * 25fps), source out = 25 + 75 = 100
    ...
    it 'encodes speed percentage in Time Remap filter' do
      generator = described_class.new([{ path: clip_a_path, speed: 0.952 }])
      xml = generator.to_xml
