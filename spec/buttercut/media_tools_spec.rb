...
# Real ffprobe against real files: the fixture wav has audio, a lavfi clip
# rendered with -an has none. Uses the ffmpeg/ffprobe MediaTools resolves for
...
RSpec.describe MediaTools, '.audio_stream?' do
  ...
  it 'is true for a file with an audio stream' do
    expect(MediaTools.audio_stream?(File.join(__dir__, '../fixtures/media/tone_4s_48k_mono.wav'))).to be(true)
  end
