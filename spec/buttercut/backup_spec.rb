...
RSpec.describe LibraryBackup do
  ...
  def zip_entries(archive)
    if Platform.command_available?('unzip')
      listing, _err, status = Open3.capture3('unzip', '-Z1', archive)
      expect(status).to be_success
      listing.split("\n")
    elsif (bsdtar = Platform.windows_system_tar)
      listing, _err, status = Open3.capture3(bsdtar, '-tf', archive)
      expect(status).to be_success
      listing.split("\n")
    else
  ...
  describe '#backup_all' do
    ...
    context 'when Apple Archive (aa) is available' do
      before do
        # The example runs the real `aa` CLI, which only exists on macOS.
  ...
  describe '.resolve_backups_dir' do
    it 'uses the override when provided' do
      expect(LibraryBackup.resolve_backups_dir(project_root: temp_dir, override: '/tmp/x')).to eq(File.expand_path('/tmp/x'))
    end
