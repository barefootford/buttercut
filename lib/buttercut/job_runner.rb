...
# (see job.rb); the runner is the single writer, so even though jobs run in
# parallel, every library.yaml mutation is serialized behind one mutex.
#
...
class JobRunner
    ...
    workers = Array.new([cap, jobs.size].min) do
      Thread.new do
        while (job = next_job(queue))
          log('start', job)
          result = run_one(job)
          @write_mutex.synchronize { sink << result }
  ...
  # serialized and counts as part of the job: if recording fails, the job fails.
  def run_one(job)
    mark_active(job)
    job.perform
    @write_mutex.synchronize { @library.complete!(job.field, [job.clip], announce: false) }
