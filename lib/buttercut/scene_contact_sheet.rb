#!/usr/bin/env ruby
# Generate thumbnail contact sheets for the detect-scenes skill.
# Distinct from lib/buttercut/contact_sheet.rb (which is the general-purpose
# contact sheet used by analyze-video) — this one is lighter and tuned for
# AI scene-transition detection (short interval, small thumbs, fast).
#
# Usage:
#   ruby lib/buttercut/scene_contact_sheet.rb <video> [--start S] [--end E] [--interval I] [--output PATH]

require 'json'
require 'optparse'
require 'shellwords'

module SceneContactSheet
  THUMB_WIDTH = 240
  THUMB_HEIGHT = 135
...
if __FILE__ == $0
  ...
  options[:output_path] ||= "/tmp/scene_contact_sheet_#{File.basename(video_path, '.*')}.jpg"

  result = SceneContactSheet.generate(video_path, **options)
  puts result.to_json
end
