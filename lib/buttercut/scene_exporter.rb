...
#
# Usage:
#   ruby lib/buttercut/scene_exporter.rb <scenes.yaml|"glob"> [editor] [--windows-file-paths] [--handles 0.5] [--sequence-fps 25]
...
def main
  if ARGV.length < 1
    puts "Usage: #{$0} <scenes.yaml|\"glob\"> [editor] [--windows-file-paths] [--handles N] [--sequence-fps N]"
  ...
  handles = ARGV[handles_index + 1].to_f if handles_index && ARGV[handles_index + 1]

  sequence_fps = nil
  fps_index = ARGV.index('--sequence-fps')
  sequence_fps = ARGV[fps_index + 1].to_i if fps_index && ARGV[fps_index + 1]
  ...
  scene_files = scenes_input.include?('*') ? Dir.glob(scenes_input) : [scenes_input]
