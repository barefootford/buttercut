...
class Export
  ...
  def library_name(roughcut_path)
    # Normalize only for matching — Windows Ruby handles backslash paths in
    # file I/O natively, so the original path stays untouched elsewhere.
    roughcut_path.tr('\\', '/')[%r{libraries/([^/]+)/cuts}, 1] ||
