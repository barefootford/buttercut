---
name: setup
description: Sets up a Mac or Windows PC for ButterCut. Installs all required dependencies (Homebrew on a Mac, Ruby, Python, FFmpeg, WhisperX). Use when user says "install buttercut", "set up my mac", "set up my pc", "get started", "first time setup", "install dependencies" or "check my installation".
---

# Skill: Setup

Sets up a Mac or Windows PC for ButterCut. On a Mac, two installation paths are available based on user preference.
...
- `MINGW…` / `MSYS…` / `CYGWIN…` → Windows (Git Bash). Read and follow `skills/setup/windows-setup.md` — it has its own check-current-state flow — then come back for Step 5 (the error-reporting question).
