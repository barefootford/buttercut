...
**Requirements:** Windows 10 (version 1909 or later) or Windows 11, any edition.
...
ButterCut's contact-sheet pipeline burns timestamps onto frames with the `drawtext` filter, so the build must include it. Rather than whatever version winget has today, ButterCut installs a pinned, checksum-verified build (gyan.dev's essentials release — the same vendor behind winget's FFmpeg package) into the repo's `dependencies/` folder, where ButterCut looks before PATH. Run from the buttercut directory:

```bash
bash scripts/install_ffmpeg.sh
```

The script prints `ffmpeg + ffprobe <version> installed into dependencies/ (checksums verified)` on success (or "already installed" — also success). It verifies drawtext itself, so no extra checks are needed, and nothing goes on PATH.

If it fails with a download error (server unreachable), tell the user the FFmpeg download server is unreachable and to retry later. If it fails with a **SHA-256 mismatch**, stop setup and tell the user — never work around a checksum failure by installing the binary anyway.
...
FFMPEG_W=$(cygpath -w "$PWD/dependencies/ffmpeg.exe")     # pinned build from Step 5
```
...
- **`winget` not found**: Install "App Installer" from the Microsoft Store (ships with Windows 10 1809+ / all Windows 11), or download installers directly: rubyinstaller.org (Ruby+Devkit 3.3), python.org (3.12)
- **FFmpeg download fails**: The build server may be briefly down — re-run `bash scripts/install_ffmpeg.sh`, and check https://www.gyan.dev/ffmpeg/builds/ in a browser
- **FFmpeg SHA-256 mismatch**: Re-run `bash scripts/install_ffmpeg.sh` once (a corrupted download is possible); if it persists, stop and tell the user — do not install the binary anyway
- **`drawtext` missing**: A PATH ffmpeg without drawtext is probably being used because the `dependencies/` copy is missing — run `bash scripts/install_ffmpeg.sh`
...
- **WhisperX import errors**: the pins in `requirements.txt` are the tested combination; recreate the venv (`rm -rf ~/.buttercut/venv`) and redo Step 6
