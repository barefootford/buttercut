...
- **Large finished output the user wants to keep** (a converted clip, an extracted audio file — anything over roughly 1 MB): never store it inside ButterCut. Write it to the user's Desktop or a path they specify, and hand it back by path. Get the Desktop from `ruby lib/buttercut/desktop.rb` rather than assuming `~/Desktop` — on Windows that's often not the one the user is looking at, and a file written to the wrong one just looks lost.
