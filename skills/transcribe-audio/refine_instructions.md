...
- **Context-named term match**: correct it when the intended term appears in `user_context` or `footage_summary` and the transcript has a close mishearing — e.g. `footage_summary` names a vineyard in Sonoma and the transcript has "Sanoma" (1→1 fix).
- **Nonsense-token match**: correct it when the token is a non-word with one clear real-world spelling implied by context, nothing else phonetically close — e.g. "Pleasantton" → "Pleasanton" (1→1 fix).
- **Self-witness rule**: correct it when the right form is spelled correctly elsewhere in the SAME transcript and the suspect token is phonetically close — e.g. "Pasific Hites" → "Pacific Heights" when other segments have "Pacific Heights".
- **Do NOT correct from general world knowledge alone**: require either a context naming or a self-witness. Lacking both, leave it — the generic phrasing (e.g. "a little market on Fillmore") may be exactly what was said; don't invent a specific name.
