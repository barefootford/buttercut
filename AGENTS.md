...
**Error capture and reporting.** When a `library.rb`, `export.rb`, or `transcribe_job.rb` command fails unexpectedly, it writes a crash dump under `errors/` (gitignored) and prints `BUTTERCUT ERROR CAPTURED: <path>`. When you see that banner — or when ButterCut misbehaves with no dump (a failed export, ffmpeg dying on a clip) — use the `report-error` skill. Never edit `lib/`, shipped `skills/`, or `scripts/` to work around a failure; temporary workarounds go in a `user-` skill. Reports are local files and are only sent to the ButterCut developers with the user's permission (standing preference in the gitignored `.buttercut_error_reporting`: ask/always/never).
