...
**Bug reports and feature requests.** Both reach the ButterCut developers through `lib/buttercut/report.rb`, and neither is ever sent without the user's permission (standing preference in the gitignored `.buttercut_reporting`: ask/always/never). Drafts are local JSON files under `reports/` (gitignored) that the user can read first.

When a `library.rb`, `export.rb`, or `transcribe_job.rb` command fails unexpectedly, it writes a crash dump under `reports/` and prints `BUTTERCUT ERROR CAPTURED: <path>`. When you see that banner — or when ButterCut misbehaves with no dump (a failed export, ffmpeg dying on a clip) — use the `report-bug` skill. Never edit `lib/`, shipped `skills/`, or `scripts/` to work around a failure; temporary workarounds go in a `user-` skill. When the user wants something ButterCut doesn't do yet, use the `request-feature` skill instead — only ever at their request, never on your own initiative.
